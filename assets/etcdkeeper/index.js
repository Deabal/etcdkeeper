...
function changeEtcdBase(record) {
  console.log(record);
  if (record.endpoint === "undefined") {
    $.messager.alert("Error", $.toJSON(err), "error");
  }
  $("#etcdAddr").textbox("setValue", record.endpoint);
  $("#uname").textbox("setValue", record.username);
  $("#passwd").textbox("setValue", record.password);
}
...
function selVersion(ver) {
  ...
  version = ver;
  connect();
  Cookies.set("etcd-version", version, {
    expires: 30,
  });
...
function init() {
  ...
  // $("#etcdBaseList").combobox("setValue", etcdBase);
  var t = $("#etree").tree({
    animate: true,
    onClick: showNode,
    //lines:true,
...
function changeMode(mode) {
  ...
  $("#mode_" + mode).append(
    '<div id="' + curIconMode + '" class="menu-icon icon-ok"></div>'
  );
...
function changeFooter(ttl, cIndex, mIndex) {
  $("#footer").html(
    "<span>TTL&nbsp;:&nbsp;" +
      ...
      "&nbsp;&nbsp;&nbsp;&nbsp;ModRevision&nbsp;:&nbsp;" +
      mIndex +
      '</span><span id="showMode" style="position: absolute;right: 10px;color: #777;">' +
      aceMode +
      "</span>"
  );
}
...
function changeTreeMode() {
  if (version === "2") {
    ...
    Cookies.set("tree-mode", treeMode, {
      expires: 30,
    });
    connect();
  }
